        ...
        <div className="flex flex-col items-center gap-10 w-full xl:mt-10 justify-center px-10 pt-16 pb-5">
            {
                OnBoardingOne.map((item,index)=>(
                    <div className="flex gap-5 md:flex-row flex-col items-start px-4 md:px-14 py-8 justify-center bg-secondaryColor rounded-md" key={index}>
            ...
            }
             {
                OnBoardingTwo.map((item,index)=>(
                    <div className="flex md:flex-row flex-col gap-14 items-center px-4 md:px-14 py-8 justify-center  rounded-md" key={index}>
          ...
          <div className="flex items-center justify-center flex-col md:flex-row  w-full">
            {
                usingCauses.map((item,index)=>(
                    <div className={`flex flex-col w-full gap-3 md:px-10 px-4 items-center justify-center py-8 h-[150px] md:h-[200px] ` } key={index}>
                        ...
                        <h1 className="md:text-2xl text-xl text-center md:text-start text-primaryText font-bold">{item.title}</h1>
                        <p className=" w-[350px] text-center md:text-start text-xl font-semibold text-headertext">{item.text}</p>
                        </div>
                ))
            }
